...
def Likelihood_second(A, hx = TH1F("ac", "ac", 10, 0, 10)):

    sum2 = 0.0      
    for i in range (1, 50):
        sum2 = sum2 + (hx.GetBinContent(i))*log(A) - A - log( Factorial( hx.GetBinContent(i)))
...
def Pseudo_Experiment(number_of_experiments = 10, hx = TH1D("fakeasdfas", "fakeasfdasa", 50, 0, 5), is_constant = 1):
    fake_experiment = TH1D("fake", "fake", 50, 0, 5)
    Likelihood_Histogram = TH1D("ll", "ll", 100, -500, -10)
    ...
    position_x = Likelihood_second(A_0, hx)    
...
#Likelihood_TwoVariables(0, 5000, 10, 0, 4000, 10, 1)
